...
class BrainfuckArray : Benchmark() {
    ...
    class Tape {
        ...
        fun inc() {
            tape[pos] = (tape[pos] + 1).toByte() // Wrapping overflow
        }
    ...
    class Program(private val text: String) {
        ...
        private val jumps: IntArray // Массив вместо Map
        ...
        init {
            ...
            commands.forEachIndexed { i, cmdByte ->
                val cmd = cmdByte.toInt().toChar() // Исправлено: toInt().toChar() вместо toChar()
                when (cmd) {
                    '[' -> stack.add(i)
                    ']' -> {
                        if (stack.isNotEmpty()) {
        ...
        fun run(): Long {
            ...
            while (pc < commands.size) {
                val cmd = commands[pc].toInt().toChar() // Исправлено: toInt().toChar() вместо toChar()
                when (cmd) {
                    '+' -> tape.inc()
                    '-' -> tape.dec()
                    '>' -> tape.advance()
