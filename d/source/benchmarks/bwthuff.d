...
private:
    ...
    void buildHuffmanCodes(HuffmanNode node, int code, int length, ref HuffmanCodes huffmanCodes)
    {
        if (node.isLeaf)
        {
            if (length > 0)
...
public:
    ...
    CompressedData compress(const ubyte[] data)
    {
        ...
        return CompressedData(bwtResult, frequencies, encoded.data, encoded.bitCount);
    }
    ...
    ubyte[] decompress(CompressedData compressed)
    {
        ...
        ubyte[] decoded = huffmanDecode(compressed.encodedBits, huffmanTree,
