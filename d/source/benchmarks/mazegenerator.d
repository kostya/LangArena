    ...
    private:
        ...
        void addRandomPaths() {
            ...
            for (int i = 0; i < numExtraPaths; i++) {
                ...
                if (this.opIndex(x, y) == Cell.Wall &&
                    this.opIndex(x - 1, y) == Cell.Wall &&
                    this.opIndex(x + 1, y) == Cell.Wall &&
                    this.opIndex(x, y - 1) == Cell.Wall &&
                    this.opIndex(x, y + 1) == Cell.Wall) {
        ...
        bool isConnectedImpl(Tuple!(int, int) start, Tuple!(int, int) goal) const {
            if (start[0] >= width_ || start[1] >= height_ ||
                goal[0] >= width_ || goal[1] >= height_) {
                return false;
            }
...
public:
    ...
    override string className() const { return "MazeGenerator"; }    
