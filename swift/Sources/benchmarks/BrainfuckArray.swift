...
class BrainfuckArray: BenchmarkProtocol {
    ...
    private func interpret(_ commands: [UInt8], _ jumps: [Int]) -> UInt32? {
        ...
        while pc < commands.count {
            ...
            case 46: // '.'
                // Calculate result: result = (result << 2) + tape[tapePtr]
                // This matches the Rust implementation
                result = result &<< 2
                result = result &+ UInt32(tape[tapePtr])
                
            case 44: // ',' - ignore for benchmark
                break
                
            default:
                // Should never happen if parseCommands worked correctly
                break
            }
