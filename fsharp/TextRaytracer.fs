...
type Vector =
    ...
    member this.Scale(s: double) =
        Vector(this.X * s, this.Y * s, this.Z * s)

    member this.Add(other: Vector) =
        Vector(this.X + other.X, this.Y + other.Y, this.Z + other.Z)

    member this.Subtract(other: Vector) =
        Vector(this.X - other.X, this.Y - other.Y, this.Z - other.Z)

    member this.Dot(other: Vector) =
...
type Color =
    ...
    member this.Scale(s: double) =
        Color(this.R * s, this.G * s, this.B * s)

    member this.Add(other: Color) =
...
type Sphere =
    ...
    member this.GetNormal(pt: Vector) = pt.Subtract(this.Center).Normalize()
    ...
    let intersectSphere (ray: Ray) (center: Vector) (radius: double) =
        ...
        if tca < 0.0 then
            ...
            if d2 > r2 then
                ...
                if t0 > 10000.0 then None else Some t0
